...
import { HardwiresHistogram } from './HardwiresHistogram';
...
interface AnalysisResults {
  overallScore: number;
  hardwireScores: HardwireScore[];
  strengths: string[];
  areasForGrowth: string[];
  recommendations: string[];
  topHardwires: HardwireScore[];
  lowestHardwires: HardwireScore[];
}

const HARDWIRE_DEFINITIONS = [
...
export function AnalysisReport({ responseId, customerEmail, onClose, onStartRound2, isRound2 = false }: AnalysisReportProps) {
  ...
  const analyzeResponses = async () => {
    ...
    if (response) {
      ...
      const hardwireScores: HardwireScore[] = HARDWIRE_DEFINITIONS.map((hw, index) => {
      ...
      const sortedByScore = [...hardwireScores].sort((a, b) => b.score - a.score);
      const topHardwires = sortedByScore.slice(0, 5);
      const lowestHardwires = sortedByScore.slice(-5).reverse();

      const overallScore = Math.round(
        hardwireScores.reduce((sum, hw) => sum + hw.score, 0) / hardwireScores.length
      );

      const strengths = hardwireScores
        .filter(hw => hw.score < 50)
        .map(hw => hw.name)
        .slice(0, 5);

      const areasForGrowth = hardwireScores
        .filter(hw => hw.score >= 65)
        .map(hw => hw.name)
        .slice(0, 5);

      const recommendations = generateRecommendations(topHardwires);
  ...
  const generateRecommendations = (topHardwires: HardwireScore[]): string[] => {
    const recommendations: string[] = [];

    topHardwires.forEach(hw => {
      if (hw.code === 'DIS') {
    ...
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4 overflow-y-auto">
      <div className="bg-white rounded-2xl shadow-2xl max-w-6xl w-full p-8 relative my-8">
        ...
        {activeTab === 'overview' ? (
          <div className="space-y-6">
            ...
            <div className="bg-white border border-gray-200 rounded-xl p-6">
              ...
              <div className="space-y-4">
                {analysis.topHardwires.map((hw, idx) => (
            ...
            <div className="bg-green-50 border border-green-200 rounded-xl p-6">
              ...
              <div className="grid md:grid-cols-2 gap-4">
                {analysis.lowestHardwires.slice(0, 4).map((hw) => (
          ...
          <div className="space-y-6">
            <HardwiresHistogram scores={analysis.hardwireScores} />
            ...
            <div className="bg-white border border-gray-200 rounded-xl p-6">
              ...
              <div className="space-y-4">
                {analysis.hardwireScores.map((hw) => (
