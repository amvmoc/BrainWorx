...
interface ComparisonResults {
  originalScores: HardwireScore[];
  newScores: HardwireScore[];
...
export function QuestionComparison({ onClose }: QuestionComparisonProps) {
  ...
  const calculateHardwireScores = (answers: Record<string, string>, questionCount: number = 50): HardwireScore[] => {
    return HARDWIRE_DEFINITIONS.map((hw, index) => {
  ...
  const handleAnalyze = async () => {
    ...
    const originalScores = calculateHardwireScores(originalAnswers, 50);
    ...
    const newScores = calculateHardwireScores(newAnswersMap, parsedQuestions.length);
        ...
        <div className="bg-white rounded-2xl shadow-2xl max-w-7xl w-full p-8 relative my-8">
                    ...
                    <tbody>
                      {comparisonResults.differences.map((diff, index) => (
                        <tr key={index} className="border-b border-gray-100 hover:bg-gray-50">
                          <td className="py-3 px-4 font-medium text-[#0A2A5E]">{diff.hardwire}</td>
          ...
          {activeTab === 'original' && (
            <div>
              ...
              <HardwiresHistogram hardwireScores={comparisonResults.originalScores} />
            </div>
          )}
          ...
          {activeTab === 'new' && (
            <div>
              ...
              <HardwiresHistogram hardwireScores={comparisonResults.newScores} />
            </div>
          )}
